...
TaskManager.prototype.stopCurrentTask = function(comment) {
  ...
  if (comment) {
    this.currentTask.description += ' ' + comment;
  }
...
TaskManager.prototype.exportTasks = function(day, exporterName) {
  return doWithDay(day, function(data) {
    try {
...
TaskManager.prototype.mergeTasks = function(day) {
  return doWithDay(day, function(data) {
    var taskMerger = new tM(data.tasks);
 ...
 */
function doWithDay(day, callback) {
  if (!day) {
...
function setupCurrentDay(taskManager, file) {
  ...
  if (data.currentId) {
    ...
    utils.printTask(taskManager.currentTask);
  }
}
